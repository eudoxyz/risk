...
var KONTINENTI = {
  ...
  'northAmerica': [].concat(_toConsumableArray(Array(9).keys())).slice(0),
  'southAmerica': [].concat(_toConsumableArray(Array(13).keys())).slice(9),
  'europe': [].concat(_toConsumableArray(Array(20).keys())).slice(13),
  'africa': [].concat(_toConsumableArray(Array(26).keys())).slice(20),
  'asia': [].concat(_toConsumableArray(Array(38).keys())).slice(26),
  'australia': [].concat(_toConsumableArray(Array(42).keys())).slice(38)
...
var menuState = {
  ...
  create: function create() {
    ...
    var message = game.add.text(game.world.centerX, game.world.centerY, 'Klikni mišem za početak', { font: '48px Arial', fill: '#F3F3F3' });
    message.anchor.setTo(.5, .5);

    var click = game.input.mousePointer.leftButton;
...
var playState = {
  ...
  drawEdge: function drawEdge(src, dst, pair) {
    var grane = game.add.graphics();
    ...
    if (pair[0] === 0 && pair[1] === 31) {
      ...
      var CURVATURE = 650;
      var MIDDLE_X = (src[0] + dst[0]) / 2;
      var MIDDLE_Y = src[1]; // y-koordinata im je ista
      var DISTANCE = dst[0] - src[0];
      var RADIUS = Math.sqrt(Math.pow(CURVATURE, 2) + Math.pow(DISTANCE / 2, 2));
  ...
  drawEdges: function drawEdges() {
    var drawn = [].concat(_toConsumableArray(Array(42).keys())).map(function (i) {
      return Array(42);
    }); // 42 x 42 niz
    for (var i = 0; i < GRAF.length; i++) {
      for (var j = 0; j < GRAF[i].length; j++) {
        if (!drawn[i][GRAF[i][j]]) {
...
var game = new Phaser.Game(WIDTH, HEIGHT, Phaser.AUTO, 'root');
